...
def download_model_if_needed():
    model_path = "saved_model_v2.pkl"  # Updated to v2
    if os.path.exists(model_path):
        size = os.path.getsize(model_path)
        if size > 10_000_000:
            try:
...
async def predict(req: PredictRequest):
    try:
        ...
        if key_player1 not in columns or key_player2 not in columns or key_surface not in columns:
            return {"error": "Invalid player or surface"}
        ...
        proba = model.predict_proba(input_df)[0]
        winner = req.player1 if proba[1] >= proba[0] else req.player2
...
async def debug_model_file():
    ...
    return {"exists": exists, "size_bytes": os.path.getsize("saved_model_v2.pkl") if exists else 0}